...
function act(action: ShortcutAction, seat?: number) { if (action === "palette") return openPalette(); if (action === "escape") return void (palette.value ? closePalette() : booting.value = false); if (action === "convene") return void store.start(); if (action === "add-seat") return void store.addPanelist(); if (action === "export") return void exportDump(); if (action === "help") return openPalette(); if (action === "focus-seat" && seat !== undefined) document.querySelector<HTMLElement>(`[data-seat-index='${seat}']`)?.focus(); }
