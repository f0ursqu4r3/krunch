...
<template>
  <div class="chamber relative flex h-full flex-col overflow-hidden" :class="{ 'effects-reduced': autoReduced }" :style="{ '--effects-intensity': effects === 'off' || autoReduced ? 0 : effects === 'max' ? 1 : .55 }">
    ...
    <div class="relative z-10 flex min-h-0 flex-1 flex-col"><SetupScreen v-if="store.phase === 'setup'" class="boot" /><RoomScreen v-else class="boot" /><VerdictScreen v-if="store.phase === 'verdict'" /></div>
