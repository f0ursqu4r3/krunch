...
<template>
  <div class="terminal-panel p-4">
    <p class="mb-2.5 font-mono text-[11px] uppercase tracking-[0.14em] text-signal">Panel presets</p>
    <div class="flex gap-2">
      <Input v-model="name" placeholder="Name this panel…" class="h-8 bg-bg-deep text-[11px]" @keydown.enter="save" />
      <Button size="xs" variant="outline" class="border-signal/50 text-signal" :disabled="!name.trim()" @click="save"><Save data-icon="inline-start" />Save</Button>
    </div>
    <ul v-if="settings.presets.length" class="mt-3 space-y-1.5 border-t border-line pt-3 font-mono text-[11px]">
      <li v-for="p in settings.presets" :key="p.id" class="flex items-center gap-2">
        <button class="flex-1 truncate text-left text-fg-muted hover:text-signal" @click="load(p.config_json)">{{ p.name }}</button>
