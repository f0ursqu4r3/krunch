...
<template>
  <aside class="log-rail terminal-panel flex min-h-0 flex-col overflow-hidden" aria-label="Event log">
    <header class="flex items-center justify-between border-b border-line px-3 py-2">
      <span class="font-mono text-[11px] uppercase tracking-[0.14em] text-brass">Proceedings</span>
    ...
    <div ref="scroller" class="min-h-0 flex-1 overflow-y-auto p-3 font-mono text-[10px] leading-relaxed" @scroll.passive="onScroll">
      ...
      <p v-for="line in store.logLines" :key="line.id" class="break-words text-fg-muted"><span class="text-fg-faint">{{ localTime(line.receipt) }}</span> <span class="text-brass">{{ line.kind }}</span> {{ line.text }}</p>
    </div>
    <form v-if="prompt" class="border-t border-brass/45 bg-brass/5 p-3" @submit.prevent="submit">
      <p class="mb-2 font-mono text-[11px] uppercase tracking-[0.12em] text-brass">The panel asks · round {{ prompt.round + 1 }}</p>
      ...
      <div class="flex gap-2"><Button size="xs" type="submit" class="bg-brass text-primary-foreground">Submit answers</Button><Button size="xs" type="button" variant="ghost" class="text-deadlock" @click="store.abandon()">Abort</Button></div>
    </form>
  </aside>
</template>
