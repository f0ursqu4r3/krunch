...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent :show-close-button="false" class="max-w-xl border-signal/50 bg-surface-2 p-0" @escape-key-down="emit('update:open', false)">
      ...
      <ListboxRoot v-model="selected" class="p-2" @update:model-value="choose">
        <ListboxItem v-for="entry in entries" :key="entry.id" :value="entry.id" as="button" class="flex w-full items-center justify-between px-3 py-2 text-left font-mono text-xs text-fg-muted outline-none data-[highlighted]:bg-signal/15 data-[highlighted]:text-signal">
