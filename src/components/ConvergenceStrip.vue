...
const tone = computed(() => telemetry.value?.effectiveRuling === "CONSENSUS" ? "text-consensus" : telemetry.value?.effectiveRuling === "DEADLOCK" ? "text-deadlock" : "text-signal");
</script>

<template>
  <section class="w-full" aria-label="Convergence telemetry">
    ...
    <div class="flex items-center gap-2">
      <div class="h-2 flex-1 overflow-hidden border border-line bg-bg-deep">
        <div class="h-full transition-[width] duration-300" :class="telemetry?.effectiveRuling === 'DEADLOCK' ? 'bg-deadlock shake' : telemetry?.effectiveRuling === 'CONSENSUS' ? 'bg-consensus' : 'bg-signal'" :style="{ width: `${Math.round((telemetry?.clusterFraction ?? 0) * 100)}%` }" />
      </div>
