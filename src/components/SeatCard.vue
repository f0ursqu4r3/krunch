...
<template>
  <Card class="rise min-h-0 gap-0 overflow-hidden border bg-surface/60 py-0 transition-colors duration-500"
    ...
    <CardHeader class="flex items-center justify-between gap-2 border-b border-line/60 px-4 py-2.5">
      <div class="flex min-w-0 items-center gap-2.5">
        <span class="size-1.5 shrink-0 rounded-full transition-colors" :class="active ? 'bg-brass candle' : 'bg-fg-faint/40'" />
        <span class="truncate font-display text-[15px] text-foreground">{{ seat.display_name }}</span>
      </div>
      <span class="shrink-0 font-mono text-[10px] uppercase tracking-[0.12em]" :class="status.cls">{{ status.label }}</span>
    </CardHeader>

    <CardContent class="min-h-0 flex-1 overflow-y-auto px-4 py-3 text-[13px] leading-relaxed text-foreground/85">
      ...
      <p v-if="l?.status === 'abstained'" class="mt-2 font-mono text-[11px] text-deadlock/90">abstained — {{ l.reason }}</p>
    </CardContent>

    <CardFooter v-if="l?.stance" class="flex-col items-stretch gap-2 border-t border-line/60 bg-bg-deep/30 px-4 py-3">
      ...
      <div class="flex items-center gap-2.5">
        ...
        <div class="h-[3px] flex-1 overflow-hidden rounded-full bg-surface-3">
          <div class="h-full rounded-full bg-brass transition-all duration-700" :style="{ width: `${(l.confidence ?? 0) * 100}%` }" />
        </div>
        <span class="w-8 text-right font-mono text-[11px] text-brass">{{ ((l.confidence ?? 0) * 100).toFixed(0) }}</span>
      </div>
    </CardFooter>
    <CardFooter v-else-if="lineCount > 0" class="justify-end border-t border-line/40 px-4 py-1.5">
