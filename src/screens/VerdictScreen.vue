...
const meta = computed(() => store.finalState === "converged" ? ["Consensus", "text-consensus"] : store.finalState === "deadlocked" ? ["Deadlock", "text-deadlock"] : [String(store.finalState ?? "Halted").replace(/^\w/, (c) => c.toUpperCase()), "text-brass"]);
...
<template>
  ...
  <div v-if="!dismissed" class="absolute inset-0 z-30 grid place-items-center bg-bg-deep/85 p-5 backdrop-blur-sm" @click.self="ready = true">
    <section class="terminal-panel relative max-h-full w-full max-w-4xl overflow-y-auto border-2 p-6" :class="ready ? 'boot' : ''">
      <button class="absolute right-3 top-3 text-fg-faint hover:text-brass" aria-label="Dismiss verdict overlay" @click="dismissed = true"><X class="size-4" /></button>
      <p class="font-mono text-[10px] uppercase tracking-[0.2em] text-brass">The ruling</p>
      <pre v-if="!ready" class="mt-5 overflow-hidden font-mono text-xs leading-6 text-fg-muted">compiling the panel's reasoning…
weighing final confidence
entering the ruling into the record</pre>
      <template v-else><p class="mt-5 font-display text-5xl sm:text-7xl" :class="meta[1]">{{ meta[0] }}</p><p class="mt-3 font-mono text-[10px] text-fg-faint">{{ store.sessionId?.slice(0, 8) }} · {{ store.rounds.length }} rounds deliberated</p><article class="mt-7 border-y border-line py-6"><div class="max-w-[68ch] text-[15px] leading-[1.7] text-foreground/90"><StreamMarkdown :text="verdictText" /></div></article><footer class="mt-6 flex flex-wrap items-center gap-2"><Button size="sm" variant="outline" class="border-brass/45 text-brass" @click="copy"><Copy data-icon="inline-start" />{{ copied ? 'Copied' : 'Copy record' }}</Button><Button size="sm" variant="outline" class="border-consensus/45 text-consensus" @click="download"><Download data-icon="inline-start" />{{ saved ? 'Saved ✓' : 'Download filing' }}</Button><Button size="sm" variant="ghost" class="text-fg-muted" @click="store.backToSetup()"><RotateCcw data-icon="inline-start" />New session</Button></footer><p v-if="saved" class="mt-2 font-mono text-[10px] text-consensus">→ {{ saved }}</p><p v-if="exportError" class="mt-2 font-mono text-[10px] text-deadlock">! {{ exportError }}</p></template>
    </section>
  </div>
  </Transition>
</template>
