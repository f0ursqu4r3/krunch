  ...
  <main class="min-h-0 flex-1 overflow-y-auto p-4 lg:p-6">
    <div class="mx-auto grid max-w-[110rem] grid-cols-[minmax(0,1fr)_19rem] gap-6">
      <div class="min-w-0 space-y-5">
        ...
        <section class="terminal-panel p-4"><header class="mb-4 flex items-center justify-between"><p class="font-mono text-[11px] uppercase tracking-[0.14em] text-brass">The panel // {{ store.panelists.length }}/6 seated</p><Button size="xs" variant="outline" :disabled="store.panelists.length >= 6" class="border-consensus/45 text-consensus" @click="store.addPanelist()"><Plus data-icon="inline-start" />Add seat <kbd class="ml-1 text-fg-faint">A</kbd></Button></header><div class="grid gap-3 xl:grid-cols-2"><SeatEditor v-if="store.mediator" :seat="store.mediator" /><SeatEditor v-for="seat in store.panelists" :key="seat.id" :seat="seat" removable @remove="store.removeSeat(seat.id)" /></div></section>
      </div>
